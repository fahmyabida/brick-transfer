...
type DeductBalanceWorker struct {
	UserBalanceUsecase domain.IUserBalanceUsecase
	Validator          validator.Validate
}

func NewDeductBalanceWorker(UserBalanceUsecase domain.IUserBalanceUsecase) DeductBalanceWorker {
	return DeductBalanceWorker{
		UserBalanceUsecase: UserBalanceUsecase,
		Validator:          *validator.New(),
	}
}

func (w DeductBalanceWorker) validateMessage(ctx context.Context, m *sqs.Message) (payload domain.Transfers, err error) {
...
func (w DeductBalanceWorker) GetHandler() func(ctx context.Context, m *sqs.Message) error {
	return func(ctx context.Context, m *sqs.Message) (err error) {
		...
		err = w.UserBalanceUsecase.DeductBalance(ctx, &payload)
		if err != nil {
